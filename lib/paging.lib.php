...
class paging
{
	function navigation($aTotal, $aUrl, $aItemsPerPage = 3, $aLinksPerPage = 5) //recibe el total de valores a ser mostrados, la direccion base a la cual ir, el número de items por página y la cantidad de páginas con números a mostrar, retorna un juego de enlaces listos para colocar en la página. Puede que haya que modificar los estilos.
	{
		if ($aTotal && $aTotal > $aItemsPerPage){
			...
			$out .= "Pagina {$current_page} de {$num_pages} -  ";
			...
			{
				$prev = $current_page - 1;
				$out .= "<a class=\"url\" href=\"{$aUrl}{$delim}page=1{$anchor}\" title=\"Principio\"> &#171;&#171; </a>";
				$out .= "<a class=\"url\" href=\"{$aUrl}{$delim}page={$prev}{$anchor}\" title=\"Anterior\"> &#171; </a>";
			}
			foreach ($pages as $page){
				if ($current_page == $page){
					$out .= "<span class=\"a\" class=\"PagingCurrent\"> {$page} </span>";
				}
				else{
			...
			if ($current_page < $num_pages){
				$next = $current_page + 1;
				$out .= "<a class=\"url\" href=\"{$aUrl}{$delim}page={$next}{$anchor}\" title=\"Siguiente\"> &#187; </a>";
				$out .= "<a class=\"url\" href=\"{$aUrl}{$delim}page={$num_pages}{$anchor}\" title=\"Ultimo\"> &#187;&#187; </a>";
			}	
			$out .= '</div>';
		}
		return $out;
